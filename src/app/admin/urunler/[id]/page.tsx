...
export default function UrunFormPage() {
  ...
  const isNew    = id === 'yeni'
  const existing = !isNew ? mockProducts.find(p => p.id === id) : null
  ...
  const [variants, setVariants]   = useState<Partial<ProductVariant>[]>(
    existing?.variants ?? [{ size: '', color: '', colorHex: '#1A1A1A', price: 0, compareAtPrice: undefined, stock: 0, images: [] }]
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="Koleksiyon">
                <select className={inputCls}>
                  <option value="">Seçin</option>
                  <option value="1">Velvet Noir</option>
                  <option value="2">Ivory Reverie</option>
                  <option value="3">Scarlet Romance</option>
                </select>
              </FormField>
            </div>
          </section>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-warm-white border border-border p-5 space-y-4">
            ...
            <button onClick={handleSave} className={`btn-primary w-full text-xs ${saved ? 'bg-green-700 border-green-700' : ''}`}>
              <Save size={13} />
          ...
          {!isNew && existing && (
            <div className="bg-warm-white border border-border p-5 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="font-light text-stone">Fiyat</span>
                  <span className="font-medium text-charcoal">{formatPrice(Math.min(...existing.variants.map(v => v.price)))}</span>
                </div>
              </div>
            </div>
          )}
